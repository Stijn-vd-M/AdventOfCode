...
namespace Day7
{
	public class Day7
	{
		...
		public void Test1()
		{
			...
			var lastCommand = Command.Unknown;

			var outputLines = new List<string>();

			foreach(var line in lines)
			{
				var m = CommandoRegex.Match(line);

				if (m.Success)
				{
					if (lastCommand == Command.List)
					{
						crawler.AddListToCurrentFolder(outputLines.ToArray());
					}

					outputLines.Clear();

					lastCommand = ParseCommand(m.Groups["command"].Value);

					if (lastCommand == Command.ChangeDirectory)
					{
						crawler.NavigateFolder(m.Groups["argument"].Value);
					}
		...
		public void Test2()
		{
			...
			var currentFreeSpace = 70000000 - crawler.BaseFolder.Size;

			var neededSpace = 30000000 - currentFreeSpace;

			var result = crawler
				.GetAllFolders()
				.Where(f => f.Size >= neededSpace)
				.OrderBy(f => f.Size)
				.First()
				.Size;
		}
		...
		private Command ParseCommand(string command)
		{
			switch(command)
			{
				case "cd": return Command.ChangeDirectory;
				case "ls": return Command.List;
