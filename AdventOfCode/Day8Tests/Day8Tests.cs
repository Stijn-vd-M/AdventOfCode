...
namespace Day8Tests
{
	public class Day8Tests
	{
		...
		public void Test_Example()
		{
			...
			{
				var guesser = new DigitGuesser(i);

				numbers.AddRange(guesser.GetOutput());
			}

			var grouped = numbers.GroupBy(a => a).ToDictionary(a => a.Key, a => a.Count());
			var uniqueNumbers =	(grouped.ContainsKey(1) ? grouped[1] : 0) +
										(grouped.ContainsKey(4) ? grouped[4] : 0) +
										(grouped.ContainsKey(7) ? grouped[7] : 0) +
										(grouped.ContainsKey(8) ? grouped[8] : 0);

			// Assert
			uniqueNumbers.Should().Be(26);
		}
